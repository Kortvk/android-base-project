...
import pages.FavoriteController
import pages.HistoryController
import pages.SearchController
...
class HomeController : Controller(), BottomNavigationView.OnNavigationItemSelectedListener {
  ...
  override fun onCreateView(inflater: LayoutInflater, container: ViewGroup): View {
    ...
    return view.apply {
      ...
      if (routerStates?.size() == 0) {
        ...
        childRouter.setRoot(RouterTransaction.with(HistoryController()))
      } else {
      ...
      fab.setOnClickListener {
        childRouter.pushController(RouterTransaction.with(SearchController(SEARCH_CONTROLLER)))
        fab.hide()
      }
    }
  }
  ...
  override fun onNavigationItemSelected(item: MenuItem): Boolean {
    ...
    when (item.itemId) {
      R.id.menu_favorite -> {
        childRouter.setRoot(RouterTransaction.with(FavoriteController()))
        return true
      }
      R.id.menu_history -> {
        childRouter.setRoot(RouterTransaction.with(HistoryController()))
